...
function EnergyTraceTabs({
  ...
  /* 将各生产单元展开为 市电/绿电 因子级明细行 */
  const procRows = useMemo(
    () =>
      stages.flatMap((s) => [
        { unit: s.name, energy: '市电', amount: s.gridKwh, convert: Math.round(s.gridKwh * ELEC_KGCE * 100) / 100, coef: ELEC_KGCE, fsource: '企业计量表计' },
        { unit: s.name, energy: '绿电', amount: s.greenKwh, convert: Math.round(s.greenKwh * ELEC_KGCE * 100) / 100, coef: ELEC_KGCE, fsource: '绿电交易凭证' },
      ]),
    [stages],
  )
  const procTotal = Math.round(procRows.reduce((s, r) => s + r.convert, 0) * 100) / 100
    ...
    <div>
      ...
      {tab === 'process' && (
        <div>
          <DataTable
            columns={[
              { key: 'unit', label: '生产单元', render: (r) => <span className="text-foreground">{r.unit}</span> },
              { key: 'energy', label: '能源类型', render: (r) => <span className={r.energy === '绿电' ? 'text-[var(--success)]' : 'text-foreground'}>{r.energy}</span> },
              { key: 'amount', label: '用量(kWh)', align: 'right', render: (r) => <span className="font-mono">{r.amount}</span> },
              { key: 'coef', label: '折标系数(kgce/kWh)', align: 'right', render: (r) => <span className="font-mono text-muted-foreground">{r.coef}</span> },
              { key: 'fsource', label: '数据来源' },
              { key: 'convert', label: '综合能耗(kgce)', align: 'right', render: (r) => <span className="font-mono text-primary">{r.convert}</span> },
            ]}
            rows={procRows}
          />
          <div className="mt-3 flex items-center justify-between rounded-lg border border-primary/30 bg-primary/5 px-4 py-2.5 text-sm">
            ...
            <span className="font-mono text-primary">{procTotal.toLocaleString()} <span className="text-xs text-muted-foreground">kgce</span></span>
          </div>
        </div>
      )}
